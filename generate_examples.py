...
from letsplot_ggprism import theme_prism, scale_color_prism, scale_fill_prism, scale_shape_prism
...
# -----------------------------------------------------------------------------
print("Generating Dose-Response example...")
np.random.seed(42)
...
# Treatment responses (inhibited, shifted EC50 to -6.2 and max response to 360)
...
# Re-arrange to DataFrame for plotting points
rows = []
...
# Create smooth sigmoid lines for plotting
...
ggsave(p1, "docs/images/dose_response.png")

# -----------------------------------------------------------------------------
# 2. ToothGrowth Violin + Boxplot with Significance Bracket
# -----------------------------------------------------------------------------
print("Generating ToothGrowth example...")
np.random.seed(42)
means = {
...
ggsave(p2, "docs/images/tooth_growth.png")

# -----------------------------------------------------------------------------
# 3. Bar Chart with Error Bars
# -----------------------------------------------------------------------------
print("Generating Bar Chart example...")
df_simple_bar = pd.DataFrame([
    {"condition": "Vehicle", "mean": 15.2, "sd": 1.8, "ymin": 13.4, "ymax": 17.0},
    {"condition": "Compound X", "mean": 32.1, "sd": 2.9, "ymin": 29.2, "ymax": 35.0},
])

p3 = (
    ggplot(df_simple_bar, aes(x="condition", y="mean", fill="condition"))
...
ggsave(p3, "docs/images/bar_chart.png")
